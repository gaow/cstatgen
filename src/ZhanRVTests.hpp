...
public:
	...
	int fit(const std::string t, bool is_binary, int n = 1000, double a = 0.05)
	{
		...
		if (t == "CMATTest") m_model = std::shared_ptr<ModelFitter>(new CMATTest(n, a));
		else if (t == "CMCFisherExactTest") m_model = std::shared_ptr<ModelFitter>(new CMCFisherExactTest());
		else if (t == "CMCTest") m_model = std::shared_ptr<ModelFitter>(new CMCTest());
		else if (t == "CMCWaldTest") m_model = std::shared_ptr<ModelFitter>(new CMCWaldTest());
		else if (t == "MadsonBrowningTest") m_model = std::shared_ptr<ModelFitter>(new MadsonBrowningTest(n, a));
		else if (t == "RareCoverTest") m_model = std::shared_ptr<ModelFitter>(new RareCoverTest(n, a));
		else if (t == "KBACTest") m_model = std::shared_ptr<ModelFitter>(new KBACTest(n, a));
		else if (t == "FpTest") m_model = std::shared_ptr<ModelFitter>(new FpTest());
		else if (t == "SkatTest") m_model = std::shared_ptr<ModelFitter>(new SkatTest(0, a, 1, 25));
		else if (t == "VariableThresholdPrice") m_model = std::shared_ptr<ModelFitter>(new VariableThresholdPrice(n, a));
		// else if (t == "VTCMC") m_model = std::shared_ptr<ModelFitter>(new VTCMC();
		else if (t == "ZegginiWaldTest") m_model = std::shared_ptr<ModelFitter>(new ZegginiWaldTest());
		else if (t == "ZegginiTest") m_model = std::shared_ptr<ModelFitter>(new ZegginiTest());
		else m_model = std::shared_ptr<ModelFitter>(new SingleVariantScoreTest());
